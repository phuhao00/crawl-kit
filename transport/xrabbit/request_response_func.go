...
func getQueueName(ctx context.Context) string {
	if args := ctx.Value(ContextKeyQueueName); args != nil {
		return args.(string)
	}
	return ""
}

func getConsumerName(ctx context.Context) string {
	if args := ctx.Value(ContextKeyConsumerName); args != nil {
		return args.(string)
	}
	return ""
}
